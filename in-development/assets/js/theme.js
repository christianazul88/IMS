...
var totalSalesEcommerce = function totalSalesEcommerce() {
  ...
  function getFormatter(params) {
    return params.map(function (_ref20) {
      ...
      return `
          ...
          <span class='text-600'>${seriesName === 'lastMonth' ? 'Current Year' : 'Previous Year'}: ${value}</span>
        `;
    }).join('<br/>');
  }
  ...
  if ($echartsLineTotalSalesEcomm) {
    ...
    var TOTAL_SALES_LAST_MONTH = `#${userOptions.optionOne}`;
    var TOTAL_SALES_PREVIOUS_YEAR = `#${userOptions.optionTwo}`;
    ...
    fetch('monthly_sales.php')
      .then(response => response.json())
      .then(data => {
        var lastYearSales = data.last_year_sales.map(Number); // Convert strings to numbers
        var currentYearSales = data.current_year_sales.map(Number);
            ...
            series: [
              {
                ...
                areaStyle: {
                  color: {
                    ...
                    y2: 1,
                    colorStops: [
                      { offset: 0, color: utils.rgbaColor(utils.getColor('primary'), 0.2) },
                      { offset: 1, color: utils.rgbaColor(utils.getColor('primary'), 0) }
                    ]
                  }
                }
              },
