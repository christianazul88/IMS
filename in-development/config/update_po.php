...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['product_id']) && is_array($_POST['product_id'])) {
        ...
        foreach ($_POST['product_id'] as $selectedProductId) {
            ...
            $check_poc = "SELECT product_id FROM purchased_order_content WHERE product_id = '$product_id' AND po_id = '$po_id' LIMIT 1";
            $result_poc = $conn->query($check_poc);
            if($result_poc->num_rows>0){
                $insert_po_content = "UPDATE purchased_order_content SET product_id = '$product_id', qty = '$current_stock' WHERE product_id = '$product_id' AND po_id = '$po_id'";
...
if($success == $product_count){
    ...
    header("Location: ../PO-logs/?success=true&elcoco=$po_id");
    exit;
} else {
    $conn->close();
