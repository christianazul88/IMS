...
while ($row = mysqli_fetch_assoc($recent_scans_result)) {
    ...
    $product_warehouse = htmlspecialchars($row['warehouse_name']);
    ...
    $product_unit_cost = $row['capital'];
    $scanned_or_not = $row['audit_status'];
    //outbounded items
    if($outbounded_yes_no === "yes"){

    }

    //items not supposed to be on this warehouse but was scanned
    if($warehouse_origin_id !== $warehouse_onscanned_id){

    }

    //items that was not supposed to be on this rack/location 
    if($warehouse_origin_id === $warehouse_onscanned_id && $item_location_origin_id !== $item_location_onscanned_id){

    }

    // scanned items condition
    if($scanned_or_not === "scanned"){

    } elseif($scanned_or_not === "pending"){
        //this are the missing items yet that is expected to be scanned
    }

    //so technically, outbounded items and $warehouse_origin_id !== $warehouse_onscanned_id are the unexpected items, they are also positive variance
