...
$filename =
    "Scanned on other warehouses audit num " .
    $audit['audit_num'] .
    "_" .
    date("Ymd_His") .
    ".csv";
...
fputcsv($output, ["AUDIT DETAILED REPORT FOR ITEMS THAT WAS SCANNED ON OTHER WAREHOUSE"]);
...
$query = "
    ...
    WHERE ita.warehouse_origin = ?

        AND ita.warehouse_onscanned != ?
...
$stmt->bind_param("ss", $warehouse_id_audit, $warehouse_id_audit);
$stmt->execute();
