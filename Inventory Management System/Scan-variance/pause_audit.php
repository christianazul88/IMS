...
if(SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pause_audit'])){
    $remarks = $_POST['remarks'];
    $pause_query = "INSERT INTO audit_assignment_logs (audit_assignment_id, `status`, date_time, user_id) VALUES (?, 'pause', NOW(), ?)";
    $stmt = $conn->prepare($pause_query);
    $stmt->bind_param("is", $audit_assignment_id, $user_id);
    $stmt->execute();
    $stmt->close();
